...
async function generate_random_deck(deck_reference, total_deck_size) {
    ...
    while (current_deck_size < total_deck_size) { //add cards to deck until full
        let random_cardID = await get_random_cardID(deck_reference);

        while (used_cardIDs.has(random_cardID)) { //get a unique cardID
...
module.exports = {
    ...
    callback: async (client: Client, interaction: CommandInteraction) => {
			...
			const api_response = await generate_random_ydke(Number(interaction.options.get("main_deck_size")?.value), Number(interaction.options.get("extra_deck_size")?.value));
